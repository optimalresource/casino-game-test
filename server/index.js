...
io.on("connection", function (socket) {
  ...
  socket.on("signin_send", (data) => {
    console.log("[DEBUG] signin_send received:", data);

    // For testing purposes, skip database query and create a mock response
    console.log("[DEBUG] Using mock response for signin testing");
    ...
    let obj = {
      uuid,
      user: data.user,
      email: data.user + "@example.com", // Mock email
    ...
    try {
      console.log("[DEBUG] Sending signin_read response:", {
        exists: true,
        obj: obj,
      });
  ...
  socket.on("signup_send", (data) => {
    console.log("[DEBUG] signup_send received:", data);

    // For testing purposes, skip database query and create a mock response
    console.log("[DEBUG] Using mock response for testing");
    ...
    try {
      console.log("[DEBUG] Sending signup_read response:", {
        exists: false,
        obj: obj,
      });
    ...
    /*
    database_config.sql =
      'SELECT * FROM casino_user WHERE email = "' + data.email + '"';
    database_config.name = "db02";
    console.log("[DEBUG] About to query database with SQL:", database_config.sql);
    database(database_config).then(function (result) {
      console.log("[DEBUG] Database query result:", result);
      if (result && result.length == 0) {
        ...
        let array = result.filter(function (x) {
          //check if there already is an email with same username
        ...
        if (array && array.length == 0) {
          ...
          try {
            io.emit("signup_read", {
              exists: false,
              obj: {},
              details: "email_yes_user_no_error",
            });
          } catch (e) {
            console.log("[error]", "signup_read :", e);
          }
          ...
          try {
            io.emit("signup_read", {
              exists: true,
              obj: {},
              details: "email_yes_user_yes_error",
            });
          } catch (e) {
            console.log("[error]", "signup_read :", e);
          }
      ...
      // Send a response even if database fails
      try {
        io.to(socket.id).emit("signup_read", { exists: false, obj: {}, details: "database_error" });
      } catch (e) {
        console.log("[error]", "signup_read error:", e);
      }
    });
    */
  });

  // New wallet verification handler
  socket.on("wallet_verify", (data) => {
    console.log("[DEBUG] wallet_verify received:", data);

    // Verify wallet data
    const { walletAddress, chainId, provider } = data;

    if (!walletAddress || !chainId || !provider) {
      console.log("[DEBUG] Invalid wallet data received");
      io.to(socket.id).emit("wallet_verify_response", {
        success: false,
    ...
    // 4. Store the wallet info in the database
    ...
    // Store wallet info in session or database
    socket.walletInfo = {
    ...
    console.log("[DEBUG] Wallet verified successfully");
    io.to(socket.id).emit("wallet_verify_response", {
      success: true,
      message: "Wallet verified successfully",
      walletInfo: socket.walletInfo,
    });
  });
  ...
  function updateMoney(user_found, x) {
    if (user_found[0]) {
      ...
      database_config.sql +=
        "INSERT INTO history_user (user_id, game_name, game_id, game_type, date, status, sum) VALUES (?, ?, ?, ?, ?, ?, ?)";
      let payload = [
        user_found[0].id,
        table_name,
        game_id,
        ...
        if (result) {
          ...
          if (users_array && users_array.length > 0) {
            try {
              let user_found = users_array.filter((x) => x.uuid === data.uuid);
              if (user_found && user_found.length > 0) {
                let table_name = data.game.table_name
                  ? data.game.table_name
                  : "";
                let table_id = data.game.table_id
                  ? data.game.table_id
                  : table_name;
    ...
    if (details.user_uid) {
      ...
      database(database_config).then(function (result) {
        if (result) {
          users_array = result;
          if (users_array && users_array.length > 0) {
            let user_found = users_array.filter(
              (x) => x.uuid === details.user_uid
            );
