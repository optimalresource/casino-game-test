...
const stripe = require("stripe")(process.env.STRIPE_SECRET_KEY); // removed the test key and added the .env variable to replace it. For test purposes, get the test key from the README.md
...
stripePayment.post("/api/stripe", jsonParser, (req, res, next) => {
  ...
  if (amount) {
    ...
    lineItems.forEach((item, index) => {
      metadata[`product_${index + 1}`] = `${item.name}: ${item.quantity} x ${
        item.price / 100
      } USD`;
    });
          ...
          if (res1) {
            ...
            createPaymentMethod(card)
              .then((res2) => {
                if (res2) {
                  paymentMethod = res2;
                  attachPaymentMethod(paymentMethod.id, customer.id)
                    .then((res3) => {
                      if (res3) {
                        ...
                        res.json({
                          type: "stripe",
                          result: "error",
                          payload: "attachPaymentMethod_error",
                        });
                      }
                    })
                    .catch((err) => {
                  ...
                  res.json({
                    type: "stripe",
                    result: "error",
                    payload: "createPaymentMethod_error",
                  });
                }
              })
              .catch((err) => {
            ...
            res.json({
              type: "stripe",
              result: "error",
              payload: "createNewCustomer_error",
            });
          }
        })
        .catch((err) => {
...
function attachPaymentMethod(paymentMethod_id, customer_id) {
  return new Promise(function (resolve, reject) {
    stripe.paymentMethods
      .attach(paymentMethod_id, { customer: customer_id })
      .then((res) => {
        resolve(res);
      })
      .catch((err) => {
