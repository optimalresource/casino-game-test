...
const initialState = {
  user: {
    profile_pic:
      getCookie("casino_profile_pic") !== ""
        ? getCookie("casino_profile_pic")
        : null,
    account_type:
      getCookie("casino_account_type") !== ""
        ? getCookie("casino_account_type")
        : null,
    device:
      getCookie("casino_device") !== "" ? getCookie("casino_device") : null,
  ...
  isMinor:
    getCookie("casino_isminor") !== "" ? getCookie("casino_isminor") : null,
  wallet: {
    isConnected: getWalletStatus().isConnected,
    address: getWalletStatus().address,
    chainId: getWalletStatus().chainId,
    provider: null,
  },
};
...
const pageSlice = createSlice({
  ...
  reducers: {
    ...
    connectWallet: (state, { payload }) => {
      ...
      state.wallet.address = payload.account;
      state.wallet.chainId = payload.chainId;
      state.wallet.provider = payload.provider;
    },
    ...
    updateWalletAddress: (state, { payload }) => {
      state.wallet.address = payload;
    },
    updateWalletChain: (state, { payload }) => {
      state.wallet.chainId = payload;
    },
